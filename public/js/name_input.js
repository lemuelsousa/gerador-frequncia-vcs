...
function createRemoveButton(onClick) {
  ...
  btn.textContent = "x";
  btn.className =
  "text-white bg-red-500 ml-6 py-2 px-3 h-8 rounded-md text-sm cursor-pointer hover:shadow-lg hover:border-red-400 hover:border hover:-translate-0.5 flex justify-center items-center shadow-md hover:bg-red-600 active:bg-red-700";
...
function createFieldWrapper() {
  ...
  wrapper.className = "flex gap-2 items-center flex-wrap";
  return wrapper;
}
  ...
  if (addBtn) {
    addBtn.addEventListener("click", () => {
      ...
      if (inputs.length > 0) {
        ...
        const errs = validate(lastInput.value);
        const errorEl = lastInput.parentElement.querySelector("small");
        if (errs.length > 0) {
          if (errorEl) {
            errorEl.textContent = `O nome deve ${errs[0]}`;
          }
          lastInput.focus();
          return;
        }
